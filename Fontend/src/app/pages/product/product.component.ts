...
// const productId = processProductId(productIds);
  if (productId) {
   ...
   // Tải tất cả các sản phẩm từ dịch vụ web
    ...
    this.http.get<BookDetail[]>(`https://localhost:7009/api/Bookdetails/`).subscribe(
    {
      next: productResponse => {
        this.book = productResponse.filter(product=>product.categoryId === this.bookdetail?.categoryId);
      },
      error: error => {
...
//   if (productId.charAt(1) === 'D') {
//     // Remove 'I' at the second position
//     return productId.substring(0, 1) + productId.substring(2);
//   } else {
//     return productId;
//   }
// }
